...
import { Plus, Search, Filter, Eye, Copy, Edit, Trash2, Mail } from 'lucide-react';
...
import { useEmailTemplates, useDeleteEmailTemplate, useDuplicateEmailTemplate } from '../hooks/use-email-templates';
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between">
          <div className="flex flex-col sm:flex-row gap-4 flex-1">
            ...
            <Select 
              ...
              onValueChange={(value) => setSelectedCategory(value === "all-categories" ? undefined : value as TemplateCategory)}
            >
                ...
                <Card key={template.id} className="hover:shadow-md transition-shadow flex flex-col">
                  <CardHeader className="pb-3 flex-shrink-0">
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        <Badge 
                          variant="secondary" 
                          className={`${getCategoryBadgeColor(template.category)} text-xs flex-shrink-0`}
                        >
                          {categoryInfo.label}
                        </Badge>
                        {template.isDefault && (
                  ...
                  <CardContent className="pt-0 flex-1 flex flex-col justify-end">
                    <div className="flex items-center justify-between text-sm text-muted-foreground mb-4">
                      <span className="truncate">Used {template.usageCount} times</span>
                      <span className="text-xs whitespace-nowrap ml-2">{new Date(template.updatedAt).toLocaleDateString()}</span>
                    </div>
